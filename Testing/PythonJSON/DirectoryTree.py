...
class DirectorySearcher(DirectoryTraverser): 
	def __init__(self, target, path=None, strict_case=False):
		...
		try:
			...
			# Note to self; __init__ shouldn't return, however, we want to catch something in case someone fucks up here. 
			# The way this code should be called is through some main function, where we getattr() (maybe), in which case we should raise errors here, and catch them later.
